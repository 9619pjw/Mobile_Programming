...
/*
  SingleLiveEvent 클래스 선언
   - LiveData는 값이 변경될 때만 옵저버(Observer)에게 콜백을 전달하지만,
     ...
     LiveData가 옵저버를 호출하여, 불필요한 Observer Event가 발생하는 경우가 생김.
     이를 방지하기 위해 LiveData를 상속하여 만들어낸 것이 SingleLiveEvent임

   - 즉 postValue나 setValue, call 등을 통하여 setValue 함수를 거쳐야만이 Observer을 통하여 데이터를 전달 할 수 있으며,
     이는 Configuration Changed 등의 LifeCycleOwner의 재활성화 상태가 와도 원치 않는 이벤트가 일어나는 것을 방지할 수 있도록 해줌
*/

class SingleLiveEvent<T> : MutableLiveData<T>() {
    /*
       AtomicBoolean 객체 생성
         - AtomicBoolean : boolean 자료형을 갖고 있는 wrapping 클래스. 멀티스레드 환경에서 동시성을 보장
         - isPending 객체는 setValue()로 새로운 이벤트를 받으면 true로 바뀌고
           그 이벤트가 실행되면 false로 돌아감 - 초기값은 false
    */
    ...
    */
    @MainThread
    override fun observe(owner: LifecycleOwner, observer: Observer<in T>) { // observer가 여러개 들어와도 오직 1개만 처리하고자 하는 매커니즘
    ...
    /* LiveData 값이 바뀌면 호출되는 함수
     - LiveData 값이 변경되면 false였던 isPending이 true로 바뀌고, observe를 호출
       즉, setValue()에서 새로운 이벤트를 받으면 isPending 이 true로 바뀌고 observe 호출하여 데이터를 변경
    */
    @MainThread
