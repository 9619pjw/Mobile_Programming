...
data class ResultCode(
    val code: String, // 메세지 코드(INFO-000, INFO-100)
    val message: String// 메세지
)
/*

 응답 결과 클래스(Json)
    - totalCnt(전체 건수), startRow(시작Row), endRow(끝Row), resultCode(응답 코드):  참고용 메타정보
    - row: 응답 결과( List 형태의 FreshData)
*/
data class ResultMeta( // @Json(name="Json-key") 어노테이션
                       // Moshi에서 JSON으로 직렬화할 때 필드 name을 나타내는데 사용함.
                       // Moshi : JSON과 객체 사이에서 직렬화와 역직렬화를 지원하는 라이브러리
    @Json(name = "totalCnt")
    var totalCnt: Int? = null,
    @Json(name = "startRow")
    var startRow: Int? = null,
    @Json(name = "endRow")                                // Json 데이터의 ("endRow")로 대체하도록 Moshi에 지시함.
    var endRow: Int? = null,    // data class의 "endRow" 필드를
    @Json(name = "result")
