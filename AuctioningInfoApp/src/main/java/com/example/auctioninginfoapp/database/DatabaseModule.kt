...
abstract class DatabaseModule : RoomDatabase() {
    ...
    /* 데이터베이스를 Singleton으로 반환하기 위한 Companion Onject 선언 */
    companion object {

        private var database: DatabaseModule? = null // database 변수 선언

        private const val ROOM_DB = "room.db" // database 이름 상수 선언

        /* 데이터베이스를 Singleton으로 반환하는 함수 선언 */
        fun getDatabase(context: Context): DatabaseModule {
            return database ?:  Room.databaseBuilder(context.applicationContext, DatabaseModule::class.java, ROOM_DB)
