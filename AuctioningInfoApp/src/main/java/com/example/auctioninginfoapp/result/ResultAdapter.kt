...
/* ResultFragment 에서 검색(농산물 공공 DB) 결과를 리사이클러 뷰에 데이터를 보여주는 어댑터  */
...
/* SaveFragment 에서 DB에 저장된 검색 결과를 리사이클러 뷰에 데이터를 보여주는 어댑터
   - 검색된 결과(SAVE_ID)를 KEY로 FreshData 테이블에서 해당 id를 찾아 화면에 출력
   - SAVE_ID로 저장된 모든 데이터를 리사이클러 뷰의 랜더링 사이즈 만큼씩 보여줌
*/
...
class ItemViewHolder(private val binding: ListItemFreshBinding) : RecyclerView.ViewHolder(binding.root) {
    ...
    fun bindItems(fresh: FreshData?) { // 검색한 경락 가격 정보를 아이템 뷰(list_item_fresh)에 바인딩하는 함수
        fresh?.let {
            binding.txtGongpanInfo.text = fresh.cprName //법인명
            binding.txtUnit.text = "${fresh.mname} > ${fresh.sname} (등급:${fresh.grade})" // 수박 > 수박(일반) (등급:특)
