...
class ResultViewModel() : ViewModel() {
    ...
    val moshi by lazy { // JSON 라이브러리 moshi 플러그인 참조
    ...
    */
    ...
    fun saveResult(context: Context, saveName: String) {
        viewModelScope.launch(Dispatchers.IO) {
            /* SaveItem 테이블에 경락가격정보 목록(id, saveTitle) 저장
                - 먼저 저장항목 리스트(SaveItem)를 만들어서 autoGenerated된 ID값을 받은다음,
                - freshdata(각각의 개별 데이터)를 리스트 ID값과 함께 저장.
             */
            DatabaseModule.getDatabase(context).freshDao().insertSave(
                // Fresh Entity에 저장할 경락가격정보의 목록을 SaveItem Entity에 저장
                ...
                resultList.value?.let { datas ->
                     ...
                     ** [Fresh] table에 저장
                       ...
                       2     1         사과               1
                             :
                     */
                    DatabaseModule.getDatabase(context).freshDao().insertFresh(datas) // Fresh Entity에 검색한 경락가격정보 저장
                }
            }
        }
    }//end of saveResult
    ...
    /* 코루틴 실행중 발생한 Exception을 처리하기 위한 CoroutineExceptionHandler 설정
       - errorHandler CoroutineExceptionHandler를 구현하며, Exception이 발생하면 선언한 람다식을 실행
    */
    private val errorHandler = CoroutineExceptionHandler { _, exception ->
        exception.message?.let { Log.e("error", it) } // 예외 발생 시 실행할 람다식
        notificationMsg.postValue(exception.message)
    }

    // Request 객체 생성 함수
    fun loadDataFromURL( selectDate: String, selectFruit: String, resultAmount: String) {
        // Request(요청) 객체 생성
        val httpUrl = NetworkModule.makeHttprequest(
            // 품종코드, 검색일자, 검색수량을 인자로 HttpUrl 객체 생성 함수를 호출하여 HttpUrl 객체를 생성
        ...
        */
        ...
        viewModelScope.launch(Dispatchers.IO + errorHandler) {
            /*  OkHTTPClient를 이용하여 네트워크 통신(경락가격 정보를 서버에 요청)
                - OkHTTPClient.newCall()의 인자로 Request 객체(request)를 전달하여 응답(response) 객체 생성
                - IO 스레드에서 네트워크 통신을 한 다음에(await로 IO 스레드가 완료될때까지 기다림)
             */
            val response = NetworkModule.clinent.newCall(httpUrl).await() // 공공 DB 서버에 요청 및 응답 받기
            Log.d("RESPONSE", "$response")

            // 응답 결과(response, Json 형식)를  List<FreshData> 형식의 객체로 변환
            // response 데이터 -> FreshData 포맷
            val freshData = response.body?.string()?.let { mapppingStringToFresh(it) } ?: emptyList()

            if(freshData.isEmpty()){ // ERROR LiveData 저장
                notificationMsg.postValue("데이터가 존재하지 않습니다.")
            }

            // resultList에 freshData를 저장(SingleLiveEvent<List<FreshData>>)
            resultList.postValue(freshData) // FreshData LiveData 저장
        }
    ...
    */
    private fun mapppingStringToFresh(jsonBody: String): List<FreshData> {
        ...
        if (freshResponse?.errorCode != null) {// 에러 메시지를 명시적으로 알려주기 위해 에러를 발생시키고,
